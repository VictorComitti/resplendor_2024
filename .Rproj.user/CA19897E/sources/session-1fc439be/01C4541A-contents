...
base_niveis_me <- dados_ME_estruturados %>%
...
merged_base <- dados_ME_estruturados %>%
  map(merge_bases)
...
gera_figura <- function(names, dados_estruturados){
  ...
  plot <-  ggplot(alpha = 0.3) +
    ...
    geom_hline(aes(yintercept = 90, col = "Cota 90"), size = 0.8)+
    geom_hline(aes(yintercept = ref, col="Cota do Terreno"), size = 1.2)+
    ggtitle(title)+
...
figuras_ME <- map2(.x = names_dados_ME,
                   .y = dados_ME_estruturados,
                   .f = gera_figura)

map2(seq_along(figuras_ME), names_dados_ME, function(i, names) {
  ggsave(
    path = "C:/Users/victo/Desktop/Comitti/resplendor2024/figurasME",
